...
class LuckyWheelText: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for angle: CGFloat in angles {
            let a = Float(angle)
            print(a)
            g.centreArcPerpendicular(text: items[i].title ?? "", context: context!, radius: CGFloat(wheelRadius - 20), angle: CGFloat(a),colour:  items[i].titleColor ?? #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), font: UIFont(name: "Arial-BoldMT", size: 14)! , clockwise: false,  numOfSections: numOfSections)
            i += 1
        }
