...
class RestaurantListCell : UITableViewCell {
  //MARK: - Properties
  var presenter: RestaurantCardPresenter? { didSet { configure() } }
...
//      self.presenter!.restaurant.isSelected.toggle()
...
//      self.actionButton.setImage(UIImage(named: self.viewModel!.selectButtonImagename)?.withRenderingMode(.alwaysOriginal), for: .normal)
  ...
  //MARK: - Helpers
  func configure(){
    guard let presenter = presenter, let config = config else { return }
    ...
    if config == .table{
      ...
      changeImageWithAninamation(imageName: presenter.selectButtonImagename)
    }else if config == .edit{
      ...
      self.actionButton.setImage(UIImage(named: presenter.selectButtonImagename)?.withRenderingMode(.alwaysOriginal), for: .normal)
    }
  }
