...
class WheelItemText: UIView {
  ...
  override func draw(_ rect: CGRect) {
    ...
    for (index, angle) in angles.enumerated() {
      ...
      var textColor = items[index].titleColor
      ...
      drawTextLayer(text: name, textColor: textColor, context: context!, angle: angle)
    }
  }
}

extension WheelItemText {
  func drawTextLayer(text: String, textColor: UIColor, context: CGContext, angle: CGFloat) {
    let textSpacing: CGFloat = 1
    // Set the text attributes
    let attributes = [
      NSAttributedString.Key.foregroundColor: textColor,
      NSAttributedString.Key.font: UIFont(name: "Arial-BoldMT", size: 14) as Any,
      NSAttributedString.Key.kern: textSpacing
